        ...
        for (i = 0; i < NUM_SP_REGISTERS; i++)
            if ((sp_register_t) i != IR && (sp_register_t) i != COND &&
                ...
                cout << reg_names[i] << " = " << dec << get_sp_register((sp_register_t) i, (stage_t) s)
                     << hex;
                cout << " / 0x" << get_sp_register((sp_register_t) i, (stage_t) s) << endl;
            }
    }
...
void sim_pipe::reset()
{
    //Setting everything to UNDEFINED

    PC_ADDER=UNDEFINED;
    IF_ID_PIPELINE_COLUMN.NPC=UNDEFINED;
    ID_EX_PIPELINE_CLOUMN.NPC=UNDEFINED;
...
int sim_pipe::get_gp_register(unsigned reg) {
    return integer_register[reg];
}

void sim_pipe::set_gp_register(unsigned reg, int value) {
    unsigned pos = 0;
    if (integer_register.empty()) {
        integer_register = create_integer_Point_registers();
    }
    //TODO may need a place holder to clear the integer register first
    integer_register[pos + reg] = value;
...
///------MY functions are dfined here---------------///
/**
 * f@2
 * creates the integer registers r0-r31
 * Behaviour::fills up the global vector of integer registers
 * TODO -Create a clear method for each time this fucntion is called
 * @return:vector<unisgned>
*/

vector<unsigned int> sim_pipe::create_integer_Point_registers() {
    //TODO placeholder for the clear method
    integer_register = vector<unsigned>(32);
    for (unsigned int i=0;i<integer_register.size(); i++) 
    {
        integer_register[i]=UNDEFINED;
    }
    return integer_register;

}

void sim_pipe::Instruction_fetch(int instruction_index) {
...
void sim_pipe::Instruction_decode(int instruction_index)
{
...
void sim_pipe::Instruction_execute(int instruction_index)
{
...
void sim_pipe::Instruction_memory(int instruction_index)
{
...
//        char2int()
            ...
            MEM_WB_REGISTER_COLUMN.LMD = char2int(data_memory);
...
void sim_pipe::Instruction_writeback(int instruction_index) {
...
bool sim_pipe::Detect_DataHazard()
{
    ...
    if(EX_STage_op!=SW && EX_MEM_PIPELINE_COLUMN.IR.src1!=UNDEFINED) {
        if (ID_Stage_op != SW&& ID_EX_PIPELINE_CLOUMN.IR.src1!=UNDEFINED) {
...
bool sim_pipe::Detect_ControlHazard()
{
...
bool sim_pipe::Detect_Branch(opcode_t opcode)
{
